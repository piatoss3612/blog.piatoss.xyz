...
**posts** (`src/content/posts/`) — `tistoryId`가 있으면 티스토리 이관분(181편), 없으면 이 블로그에서 쓴 글이다. 이 한 필드가 사이트 전체의 분기 기준이다.
...
`essay: true`면 본문까지 세리프로 조판한다. 카테고리와 무관하다 — 카테고리는 분류고 조판은 다른 축이라, 글을 옮겨도 조판이 따라 바뀌지 않는다.
...
```yaml
date: 2026-08-16
time: "23:14" # 한국 시간 HH:MM. 없으면 그날 맨 아래로 간다
```

**`date`에 시각을 합쳐 적지 말 것.** YAML은 오프셋이나 초가 없는 시각(`2026-08-16T23:14`)을 문자열로 넘기고, 그러면 `z.coerce.date()`가 빌드 머신 타임존으로 해석한다. 내 맥(KST)에서는 맞고 CI(UTC)에서는 9시간 밀려 **다른 날짜 그룹으로 넘어간다.** 필드를 나눈 게 이 때문이다.

같은 이유로 날짜 표기는 `src/lib/format.ts`가 `Asia/Seoul`에 고정해서 만든다. `getFullYear()` 같은 로컬 시간 함수를 쓰면 빌드 머신에 따라 결과가 갈린다.

`description`은 검색 결과와 공유 카드에 그대로 나간다. 본문에서 잘라 붙이지 말고 사람이 읽을 문장으로 쓴다(70~100자). 이관분 181편은 원래 본문을 160자에서 자른 것이었고 전부 다시 썼다.

**발췌** — `source`가 있으면 본문을 옮겨 적은 말로 렌더한다. 여백에 걸린 따옴표, 세리프 본문, 아래에 출처 한 줄. `comment`는 거기 덧붙이는 내 말이고 산세리프·왼쪽 정렬로 갈린다(프런트매터라 마크다운은 안 먹고 빈 줄로만 문단이 갈린다).

```yaml
source:
  author: 다자이 오사무
  book: 만년 # 단행본 → 『만년』
  work: 잎 # 선택. 그 안의 한 편 → 「잎」
  translator: 유숙자 # 선택
```

**낫표를 손으로 치지 말 것.** 표기는 `src/lib/notes.ts`의 `citation()`이 만든다(`다자이 오사무, 「잎」, 『만년』 (유숙자 옮김)`). 문자열 하나로 두면 겹낫표와 홑낫표가 어긋나고, 나중에 책별로 모을 때 그 문자열을 도로 파싱해야 한다. `source`는 `.strict()`라 모르는 키가 있으면 빌드가 선다 — 오타 난 필드가 조용히 버려지면 라벨만 사라져서 알아채기 어렵다.

**한 파일이 담는 건 인용 하나가 아니라 출처 하나다.** 같은 글에서 여러 대목을 옮겼으면 본문에서 `---`로 가른다. 위아래에 빈 줄을 둘 것 — 문단 바로 밑에 붙이면 `<hr>`이 아니라 `<h2>`가 된다. 출처가 다르면 파일을 가르고 `time`으로 순서를 준다.

`title`은 발췌에 쓰지 않는다. 책 제목을 넣으면 h1이 되어 내가 붙인 제목과 출처가 한 자리에서 섞인다. 탭과 공유 링크에 뜨는 이름은 `sourceTitle()`이 출처에서 만든다.
