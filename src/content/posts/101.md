...
- [piatoss3612/bitcoin-from-scratch](https://github.com/piatoss3612/bitcoin-from-scratch/tree/chapter13)
...
![Remaining Data 뒤에 곧바로 터진 invalid network magic](/posts/101/img/img.webp)
...
![block 0 has 1 transactions에서 멈춘 headers 파싱](/posts/101/img/img_1.webp)

 이 부분은 제가 버퍼로부터 헤더를 읽고 제거를 안 해줘 발생한 오류로 아래와 같이 수정했습니다.

![헤더 80바이트를 버퍼에서 걷어 내는 buf.Next(80)](/posts/101/img/img_2.webp)

 이렇게 수정하고 나니 headers 메시지를 정상적으로 파싱 하는 것을 확인할 수 있었습니다.

![1999번째까지 이어진 블록 헤더 해시](/posts/101/img/img_3.webp)
...
- [Bitcoin Network Snapshot - Bitnodes](https://bitnodes.io/nodes/)
...
![헤더 2000개를 받고 오류 없이 끝난 실행](/posts/101/img/img_4.webp)
...
![SimpleNode에 새로 둔 envelopes 채널](/posts/101/img/img_5.webp)

![버퍼 1000짜리 채널과 함께 띄우는 readMessages](/posts/101/img/img_6.webp)
...
![match item flag를 리틀 엔디언으로 넣도록 고친 Filterload](/posts/101/img/img_11.webp)

 그런데 이렇게 수정을 하고 filterload 메시지를 직렬화한 결과가 파이썬 예제와 동일한 것까지 확인을 했는데도 동일하게 피어 노드에서 연결을 끊어버리는 문제가 발생했습니다.

![파이썬 예제가 만든 filterload 페이로드](/posts/101/img/img_12.webp)

![같은 값이 나온 go 구현의 filterload 페이로드](/posts/101/img/img_13.webp)

 파이썬으로 실행해도 동일하게 filterload 메시지와 getheaders 메시지를 보내고 나면 피어 노드에서 연결을 끊어버리는 것 같습니다.

![파이썬 예제에서도 터지는 Connection reset!](/posts/101/img/img_14.webp)
...
![merkleblock 몇 개를 받은 뒤로 이어지는 오류](/posts/101/img/img_16.webp)
...
![마지막 줄에서 잘려 나간 바이트 스트림](/posts/101/img/img_17.webp)
...
![블록 탐색기에 18 컨펌으로 잡힌 테스트넷 거래](/posts/101/img/img_20.webp)
...
![3 컨펌으로 잡힌 연습문제 6번 거래](/posts/101/img/img_21.webp)
