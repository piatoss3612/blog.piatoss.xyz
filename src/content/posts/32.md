...
category: "Solidity"
description: "취약한 출금 함수에 공격 컨트랙트가 반복 진입해 자금을 탈취하는 과정을 따라가며, 외부 호출과 상태 변경 순서가 만드는 위험을 설명한다."
tistoryId: 32
---
## 재진입 공격 (Reentrancy Attack)이란?

![공격이 시작되기 전 두 쪽의 잔액](/posts/32/img/img.webp)
...
![1 이더를 입금해 잔액이 11이 된 BadBank](/posts/32/img/img_10.webp)

![입금에 이어 호출하는 withdraw()](/posts/32/img/img_11.webp)
...
![재진입이 도는 사이 기울어지는 잔액](/posts/32/img/img_2.webp)

 결국에는 가지고 있는 모든 이더를 Bob에게 넘겨주게 됩니다.

![0 ETH가 된 BadBank와 11 ETH가 된 Bob](/posts/32/img/img_3.webp)
...
![attack()에서 시작해 receive()까지 이어지는 호출](/posts/32/img/img_4.webp)

 그리고 만약 BadBank의 잔액이 1 이더보다 많거나 같다면 다시 withdraw 함수를 호출하여 실행경로가 withdraw 함수로 재진입하게 됩니다.

![종료 조건을 만날 때까지 도는 withdraw–receive 고리](/posts/32/img/img_5.webp)
...
![deposit()이 남긴 Attack 주소의 잔액](/posts/32/img/img_6.webp)
...
![이더 전송이 receive()를 깨워 다시 withdraw()로](/posts/32/img/img_7.webp)
...
![잔액이 그대로라 끝없이 이어지는 재진입](/posts/32/img/img_8.webp)
...
![재진입이 끝날 때까지 실행되지 않는 잔액 초기화](/posts/32/img/img_9.webp)
...
- [Contracts — Solidity 0.8.17 documentation](https://docs.soliditylang.org/en/v0.8.17/contracts.html#receive-ether-function)
- [How does reentrancy work?](https://ethereum.stackexchange.com/questions/145049/how-does-reentrancy-work)
